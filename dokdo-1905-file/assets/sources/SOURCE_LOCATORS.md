...
- **1877 기죽도약도**: 재사용 권리는 확인했다. 공식 item 페이지도 `Image / Browse`가 있음을 확인했고 국립공문서관이 IIIF를 제공한다는 점도 확인했다. 그러나 item `3018187`에 대응하는 **item-specific IIIF manifest/image identifier가 검색 결과에 노출되지 않고, 현재 자동 접근은 item page에서 403으로 막혀** 공식 이미지 URI를 안정적으로 추출하지 못했다. 제3자 복제본의 파일을 canonical asset으로 대신 넣지 않는다. 검색 가능한 연구문헌에서는 과거 열람 경로 `https://www.digital.archives.go.jp/img/3018187`도 확인되지만, 현재 자동 접근에서는 이 경로 역시 403이다.
