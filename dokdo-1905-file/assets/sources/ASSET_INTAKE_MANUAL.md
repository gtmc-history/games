...
### 페이지 탐색 단서 — frame 번호가 아님

수동 탐색 시간을 줄이기 위한 **확인용 단서**다. 아래 정보만으로 frame/page를 미리 확정하지 않는다.

- 일본국제문제연구소 2022년 보고서의 일건 문서 번각은 지도를 `L 磯竹島略圖`로 별도 구분한다.
- 같은 보고서의 주기는 현재 국립공문서관 전자판 배열을 `태정관 내부 결재서 → 시마네현 문의 → 을 제28호 → 원유의 대략 → 제1~4호 → 내무성 문의 → 磯竹島略圖` 순으로 설명한다. 즉 **지도는 현재 전자판에서 뒤쪽에 놓인다는 탐색 힌트**는 얻을 수 있다.
- 다만 이 설명은 viewer의 안정적인 frame/image identifier가 아니다. 공식 뷰어에서 지도를 눈으로 확인한 뒤에만 page/frame을 잠근다.
- 올바른 지도인지 확인할 때 찾을 원문 표기는 다음과 같다.
  - `磯竹島略圖`
  - `松島`
  - `磯竹島`
  - `隠岐 島後 福浦`
  - `朝鮮國`
- 거리·방향 설명에는 `八十里許`, `四十里許`, `五十里許` 등의 표기가 보인다.

국사편찬위원회 「사료로 본 한국사」도 1876년 시마네현이 내무성에 올린 문의에 `原由之大畧`과 도면을 첨부했고 그 도면이 `磯竹島略圖`라고 설명한다. 이 자료는 **문서 관계 교차검증용**이며, 게임용 canonical 이미지 파일 자체는 일본 국립공문서관 공개본에서 받는다.
...
국회도서관 독도 연표와 현대 연구문헌에서도 이 사건일·문구·JACAR record를 교차 확인할 수 있으나, **정확한 frame locator는 JACAR 공식 뷰어에서 직접 확인한 값만 잠근다.**
