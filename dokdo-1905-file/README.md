...
1. **S2 기죽도약도 실제 이미지 상호작용**
   ...
   - JACAR cross-locator는 `A07060000300`; 별도 『太政類典』 대조본은 `A07060000100`이다.
   - 국립공문서관 Digital Archive 디지털 콘텐츠의 재사용 권리 blocker는 해소했다.
   - 다만 현재 자동 접근 환경에서는 실제 지도 page / IIIF image identifier를 안정적으로 추출하지 못했다. 문서 배열로 페이지를 추정하지 않는다.
   - 실제 원 공개본 local crop과 명칭·상대 위치 hotspot을 넣기 전에는 lifecycle을 올리지 않는다.

2. **1904 군함 쓰시마 전시일지 자산**
   - JACAR ref: `C09050402800`
   - 1904년 11월 20일 유선 전신소 설치 적합성 조사 사실과 문서 단위 정본 locator는 확인했다.
   - 실제 게임에 사용할 정확한 스캔 프레임 번호는 아직 확정하지 않았다.
   - 방위성 방위연구소 소장 이미지의 2차 이용은 허가 확인 전까지 저장소에 복제하지 않는다. 텍스트·메타데이터 기반 플레이는 유지 가능하다.

3. **학생/교사 사용성 검수**
   - 기술적 branch browser QA는 통과했다.
   - 실제 학생이 자료 분량·장면 목표·관계 보드의 의미를 이해하는지는 수업 사용성 테스트가 별도 필요하다.
