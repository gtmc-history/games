...
function renderS2(){
  setSceneMeta(2,'1877년 태정관 지령','“죽도 외 1도”라는 직접 문구와, 그 두 번째 섬의 현재 지명 식별은 어디까지 구분해야 할까?',['지령 본문에서 “죽도 외 1도”의 처리 내용을 확인한다.','첨부 지도에서 명칭과 상대 위치 단서를 확인한다.','현재 지명 식별은 해석 메모로 따로 저장하거나 판단을 보류한다.'],'1900년 대한제국 기록을 확인한다');seen('dajokan1877');
  const mapReady=()=>new Set(S.mapClues||[]).size>=2;
  $('#sceneContent').innerHTML=sourceCard('dajokan1877')+`<div class="task-card"><h3>1단계 · 본문 직접 문구</h3><button id="direct1877" class="evidence-btn">“죽도 외 1도”가 일본과 관계없다는 취지로 처리하라고 했다.</button></div><div class="map-dev"><span class="dev-badge">DEV PLACEHOLDER · 실제 기죽도약도 크롭으로 교체 필요</span><h3>2단계 · 첨부 지도 단서</h3><p>정식 이미지 파일이 들어오기 전까지 위치·명칭 읽기 조작만 검증하는 도식입니다. 최종 BUILD에서는 일본 국립공문서관의 원 공개본 크롭 위에서 같은 단서를 선택합니다.</p><div class="map-canvas" aria-label="기죽도약도 개발용 위치 도식"><button class="island big" data-map="울릉도에 해당하는 큰 섬 표기">큰 섬</button><button class="island small" data-map="그 주변의 두 번째 섬 표기와 상대 위치">두 번째 섬</button></div><div class="map-note"><a href="https://www.digital.archives.go.jp/item/en/3018187" target="_blank" rel="noopener noreferrer">일본 국립공문서관 정본 페이지 보기 ↗</a></div></div><div class="task-card"><h3>3단계 · 식별은 해석층으로 저장</h3><p class="task-help">대한민국 외교부·동북아역사재단의 교육 자료는 첨부 지도 단서를 근거로 “외 1도”를 현재 독도로 식별합니다. 반면 이 현재 지명 식별은 지령 본문에 직접 적힌 문구가 아니므로, 본문 사실과 분리해 기록합니다.</p><div class="two-col"><button id="identify1877" class="evidence-btn interp" data-v="첨부 지도와 한국 공공교육 자료의 해석을 근거로 ‘외 1도’를 현재 독도로 식별한다.">지도·해석을 근거로 식별</button><button id="hold1877" class="evidence-btn interp" data-v="지령 본문만으로 현재 지명을 직접 확정하지 않고, 지도와 후대 해석이 필요한 식별 문제로 보류한다.">현재 지명 식별은 보류</button></div><button id="hint1877" class="ghost" type="button">지도 읽기 힌트</button></div>`;
  ...
  $$('.interp').forEach(b=>b.onclick=()=>{if(b.disabled)return;$$('.interp').forEach(x=>x.classList.toggle('selected',x===b));S.outsideOneStatus=b.dataset.v;addNote('“죽도 외 1도”가 일본과 관계없다는 취지의 처리','1877 직접 문구');addNote(S.outsideOneStatus,'1877 식별 메모');save();renderSide();ready();feedback('본문의 직접 문구와 현재 지명 식별을 서로 다른 층으로 저장했습니다. 이 해석 선택에는 단일 정오 판정을 붙이지 않습니다.')});
  $('#hint1877').onclick=()=>{uniqPush(S.hintUsed,'1877-map');feedback('힌트: 본문 표현만 보지 말고, 첨부 지도에 적힌 섬 이름과 두 섬의 상대 위치·거리 정보를 함께 읽어보세요.');save()};
  ready();
